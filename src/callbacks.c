...
void entree_incident_changed( GtkEditable *editable, gpointer user_data ){
	...
	if( extra ){
		...
		if( atoi(sIncident) >= 0 && atoi(sIncident)<= 90){

			angleIncident = atoi(sIncident);
			...
			angleRefracted = computeRefracted( widgets->repere );

			sprintf(string, "%2.2lf", angleRefracted);

			gtk_entry_set_text(GTK_ENTRY(widgets->sortie_refracte), string);
			gtk_entry_set_text(GTK_ENTRY(widgets->sortie_reflechi), sIncident);
...
void entree_V1_changed( GtkEditable *editable, gpointer user_data ){
	...
	if( extra ){
		...
		if( atoi(sV1) >= VITESSE_MIN && atoi(sV1) <= VITESSE_MAX){
			...
			angleRefracted = computeRefracted( GTK_WIDGET( widgets->repere ) );
...
void entree_V2_changed( GtkEditable *editable, gpointer user_data ){
	...
	if( extra ){
		...
		if( atoi(sV2) >= VITESSE_MIN && atoi(sV2) <= VITESSE_MAX){
